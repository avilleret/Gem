...
gem::plugins::texin :: ~texin(void) {}

static gem::PluginFactoryRegistrar::dummy<gem::plugins::texin> fac_texindummy;

#include "Gem/Exception.h"
#include "Gem/RTE.h"

namespace gem { namespace plugins {
  class texinMeta : public gem::plugins::texin {
  private:
    static texinMeta*s_instance;
    std::vector<gem::plugins::texin*>m_allHandles, // all available handles
      m_selectedHandles; // handles with the currently selected codec
    gem::plugins::texin*m_handle; // currently opened handle (or NULL)
    std::vector<std::string>m_ids; // list of handle names
    std::string m_codec; // currently selected codec

    struct codechandle {
      codechandle(gem::plugins::texin*h, const std::string c):handle(h), codec(c) {}

      gem::plugins::texin*handle;
      std::string codec;
    };
    std::map<std::string, std::vector<codechandle> >m_codechandle;
    std::vector<std::string>m_codecs;

    void addCodecHandle(gem::plugins::texin*handle, const std::string codec) {
    ...
    bool addPlugin( std::vector<std::string>available, std::string ID=std::string("")) {
      ...
      if(!ID.empty()) {
        ...
        if(std::find(available.begin(), available.end(), ID)!=available.end()) {
          ...
          verbose(2, "Gem::texin: backend '%s' unavailable", ID.c_str());
          return false;
        }
      } else {
      ...
      for(i=0; i<id.size(); i++) {
        std::string key=id[i];
        verbose(2, "Gem::texin: trying to add '%s' as backend", key.c_str());
        if(std::find(m_ids.begin(), m_ids.end(), key)==m_ids.end()) {
          ...
          try {
            ...
            handle=NULL;
            verbose(1, "Gem::texin: cannot use texin plugin '%s': %s", key.c_str(), x.what());
          }
          if(NULL==handle)continue;
          m_ids.push_back(key);
          m_allHandles.push_back(handle);
          count++;
          verbose(2, "Gem::texin: added backend#%d '%s'", (int)(m_allHandles.size()-1), key.c_str());
        }
      }
      return (count>0);
    }
...
gem::plugins::texin*gem::plugins::texin::getInstance(void) {
  texin*result=new texinMeta();
  return result;
}
