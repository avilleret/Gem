...
Base Class for Texout Capture Plugins
...
#ifndef _INCLUDE__GEM_PLUGINS_TEXOUTBASE_H_
#define _INCLUDE__GEM_PLUGINS_TEXOUTBASE_H_
...
/*-----------------------------------------------------------------
  ...
  CLASS
	...
	a OS-indendent parent-class for grabbing texout-frames
...
namespace gem { namespace plugins {
 class GEM_EXTERN texoutBase : public texout {
  public:
    /**
 ...
 protected:
    /**
     * open the texout-device
     * the exact texout device is either the default one or has to be set prior with setDevice()
    ...
    /**
     * close the texout device, freeing all resources
    ...
    /**
     * Start up the texout device (called on startRendering)
     * \return FALSE is something failed, TRUE otherwise
     */
    virtual bool	    	startTransfer(void) = 0;

    /**
     * Stop the texout device (called on stopRendering)
    ...
    /**
     * Stops the texout device and if it was running restarts it
 ...
 public:
    ...
    //
    texoutBase(const std::string name);

    //////////
    // Destructor
    virtual ~texoutBase(void);
  ...
  protected:
    ...
    // if numlocks>0 we will use a thread to capture the image create <numlocks> mutexes
    //
    texoutBase(const std::string name, unsigned int numlocks);
  ...
  public:
    // for pix_texout: query whether this backend provides access to this class of devices
    // (e.g. "dv")
