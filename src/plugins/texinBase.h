...
Base Class for Texin Capture Plugins
...
#ifndef _INCLUDE__GEM_PLUGINS_TEXINBASE_H_
#define _INCLUDE__GEM_PLUGINS_TEXINBASE_H_
...
/*-----------------------------------------------------------------
  ...
  CLASS
	...
	a OS-indendent parent-class for grabbing texin-frames
...
namespace gem { namespace plugins {
 class GEM_EXTERN texinBase : public texin {
  public:
    /**
 ...
 protected:
    /**
     * open the texin-device
     * the exact texin device is either the default one or has to be set prior with setDevice()
    ...
    /**
     * close the texin device, freeing all resources
    ...
    /**
     * Start up the texin device (called on startRendering)
     * \return FALSE is something failed, TRUE otherwise
     */
    virtual bool	    	startTransfer(void) = 0;

    /**
     * Stop the texin device (called on stopRendering)
    ...
    /**
     * Stops the texin device and if it was running restarts it
 ...
 public:
    ...
    //
    texinBase(const std::string name);

    //////////
    // Destructor
    virtual ~texinBase(void);
  ...
  protected:
    ...
    // if numlocks>0 we will use a thread to capture the image create <numlocks> mutexes
    //
    texinBase(const std::string name, unsigned int numlocks);
  ...
  public:
    // for pix_texin: query whether this backend provides access to this class of devices
    // (e.g. "dv")
